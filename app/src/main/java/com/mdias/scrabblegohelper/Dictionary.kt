...
class Dictionary (context: Context) {
    ...
    fun findScoredWords (shelf: String, otherLetters: String): ArrayList<ScoredWord> {
        val permutations: List<String> = getValidPermutations(shelf+otherLetters)
            .distinct().toList()

        val scoredWords: ArrayList<ScoredWord> = ArrayList(permutations.map {
                word: String -> ScoredWord(word)
        }.sortedByDescending {
                scoredWord: ScoredWord -> scoredWord.score
        }.toList())
    ...
    private fun getPerm(word: String, perm: String, result: ArrayList<String>) {
